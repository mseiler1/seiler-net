...
# =====================================================================
# 3. HELPER FUNCTIONS: PERMUTATION & MASKED RMSE
# =====================================================================
...
def compute_masked_rmse(model, dataloader, device, threshold=LAND_THRESHOLD):
    """
    Computes overall RMSE, Land RMSE (LWI >= threshold), and Water RMSE (LWI < threshold).
    """
    model.eval()
    
    total_sq_err_overall = 0.0
    total_count_overall = 0
    
    total_sq_err_land = 0.0
    total_count_land = 0
    
    total_sq_err_water = 0.0
    total_count_water = 0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Overall squared error accumulation
            total_sq_err_overall += sq_err.sum().item()
            total_count_overall += sq_err.numel()
            
            # Land vs. Water squared error accumulation
            if lwi is not None:
                lwi = lwi.to(device)
                if lwi.ndim == 3:
                    lwi = lwi.unsqueeze(1)
                
                land_mask = (lwi >= threshold)
                water_mask = (lwi < threshold)
                
                total_sq_err_land += sq_err[land_mask].sum().item()
                total_count_land += land_mask.sum().item()
                
                total_sq_err_water += sq_err[water_mask].sum().item()
                total_count_water += water_mask.sum().item()
                
    # 2. Take square root of the mean squared error
    rmse_overall = np.sqrt(total_sq_err_overall / max(total_count_overall, 1))
    rmse_land = np.sqrt(total_sq_err_land / total_count_land) if total_count_land > 0 else np.nan
    rmse_water = np.sqrt(total_sq_err_water / total_count_water) if total_count_water > 0 else np.nan
...
print("Calculating baseline RMSE for USA...")
base_usa = compute_masked_rmse(model, usa_loader, device)

print("Calculating baseline RMSE for Amazon...")
base_amazon = compute_masked_rmse(model, amazon_loader, device)

print("\n*** BASELINE RESULTS ***")
print(f"USA    -> Overall: {base_usa['rmse_overall']:.6f} | Land: {base_usa['rmse_land']:.6f} | Water: {base_usa['rmse_water']:.6f}")
print(f"Amazon -> Overall: {base_amazon['rmse_overall']:.6f} | Land: {base_amazon['rmse_land']:.6f} | Water: {base_amazon['rmse_water']:.6f}\n")


# =====================================================================
# RUN PFI FOR USA (RMSE)
# =====================================================================

NUM_REPEATS = 20

usa_pfi_results = []
base_off = base_usa['rmse_overall']
base_lnd = base_usa['rmse_land']
base_wtr = base_usa['rmse_water']
...
for i, name in enumerate(feature_names):
    ...
    for r in range(NUM_REPEATS):
        # 1. Permute single feature channel
        ...
        # 3. Compute degraded RMSE
        m = compute_masked_rmse(model, perm_loader, device)

        # 4. Calculate performance drop (Delta RMSE = Degraded - Baseline)
        overall_deltas.append(m['rmse_overall'] - base_off)
        land_deltas.append(m['rmse_land'] - base_lnd)
        water_deltas.append(m['rmse_water'] - base_wtr)
...
df_usa_pfi.to_csv("USA_PFI_Land_Water_RMSE.csv", index=False)
print("\nSaved USA PFI results to USA_PFI_Land_Water_RMSE.csv\n")


# =====================================================================
# RUN PFI FOR AMAZON (RMSE)
# =====================================================================

amazon_pfi_results = []
base_off_amz = base_amazon['rmse_overall']
base_lnd_amz = base_amazon['rmse_land']
base_wtr_amz = base_amazon['rmse_water']
...
for i, name in enumerate(feature_names):
    ...
    for r in range(NUM_REPEATS):
        ...
        # 4. Calculate performance drop (Delta RMSE = Degraded - Baseline)
        overall_deltas.append(m['rmse_overall'] - base_off_amz)
        land_deltas.append(m['rmse_land'] - base_lnd_amz)
        water_deltas.append(m['rmse_water'] - base_wtr_amz)
...
df_amazon_pfi.to_csv("Amazon_PFI_Land_Water_RMSE.csv", index=False)
print("\nSaved Amazon PFI results to Amazon_PFI_Land_Water_RMSE.csv")
